...
public class DriverReportCreateThread extends Thread {
    @Override
    public void run(){
        ClientThread thread = new ClientThread(1132, "Driver");
        thread.start();
        try {
            thread.join();
            ...
            document.add(new Paragraph(String.format("Average distance traveled by all drivers: %s meters.", result.get(0)[0]), f));
            document.add(new Paragraph(String.format("Average time spent by all drivers: %s seconds.",result.get(0)[1]), f));
